...
**구조적 한계:** Agent UI에서 `<input type=file>`로 이미지를 선택해도 브라우저 메모리에만 존재하며 `/mnt/user-data/uploads/`에 저장되지 않습니다. 대화창에 직접 첨부해야만 저장됩니다.

**정확한 흐름:**
1. Notion 페이지 생성 완료
2. Claude가 안내: **"서버 스펙 등 스크린샷이 있으면 지금 이 대화창에 쳊부해주세요"**
3. 사용자가 대화창에 이미지 첨부 → `/mnt/user-data/uploads/`에 저장됨
4. Claude가 `view` 도구로 해당 경로에서 직접 읽어 분석
5. 분석 결과를 Notion 해당 섹션에 업데이트 (notion-update-page)

**이미지 종류별 처리:**
- 서버 스펙 이미지 → 🖥️ 서버 스펙 섹션에 `| 항목 | 내용 |` 표로 삽입
- 기타 스크린샷 (성능 모니터, 이벤트 로그 등) → 내용 파악 후 적절한 섹션에 분석 결과 삽입
- 여러 이미지 → 순서대로 모두 처리

**Agent UI 이미지 영역:** 첨부할 파일명을 사전 입력하는 용도로만 사용. 실제 이미지는 항상 대화창에 직접 첨부.
